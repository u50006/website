...
// Enroll a student in a course.  Adds the courseId to the user's
// `courses` array if it is not already present and persists the change.
// Also updates the stored currentUser object so the UI refresh reflects
...
function enrollCourse(userId, courseId) {
  ...
  if (!user.courses.includes(courseId)) {
    ...
    saveData(data);
    // update the cached currentUser in localStorage so the dashboard
    // can display the new course without requiring a full reload
    const current = JSON.parse(localStorage.getItem('currentUser'));
    if (current && current.id === user.id) {
      current.courses = user.courses;
      localStorage.setItem('currentUser', JSON.stringify(current));
    }
  }
}

// Remove a module from a course.  This helper is used on the
// instructor-facing course page to allow deletion of a module by index.
function removeModule(courseId, moduleIndex) {
  const data = getData();
  const course = data.courses.find(c => c.id === courseId);
  if (!course || !course.modules || moduleIndex < 0 || moduleIndex >= course.modules.length) return;
  course.modules.splice(moduleIndex, 1);
  saveData(data);
}

// Remove an entire quiz from a course.  This is used by teachers to
// delete quizzes they no longer need.  It filters the quizzes array
// based on the provided quizId.
function removeQuiz(courseId, quizId) {
  const data = getData();
  const course = data.courses.find(c => c.id === courseId);
...
// Remove an assignment from a course.  This helper allows teachers
// to delete assignments by index.  It checks bounds and persists
// the updated data structure.  Used on the course page.
function removeAssignment(courseId, assignmentIndex) {
  const data = getData();
  const course = data.courses.find(c => c.id === courseId);
  if (!course || !course.assignments || assignmentIndex < 0 || assignmentIndex >= course.assignments.length) return;
...
function renderStudentDashboard() {
  ...
  // Render progress chart if the student has courses
  ...
  if (availableContainer) {
    ...
    const availableCourses = data.courses.filter(c => !user.courses.includes(c.id));
    if (availableCourses.length === 0) {
      availableContainer.textContent = 'You are enrolled in all available courses.';
    } else {
      availableCourses.forEach(course => {
        const card = document.createElement('div');
        card.className = 'card';
        // display teacher name for context
        const teacherName = data.users.find(u => u.id === course.teacher)?.name || 'Unknown';
        card.innerHTML = `<h3>${course.title}</h3><p>Instructor: ${teacherName}</p>`;
        const btn = document.createElement('button');
        btn.className = 'button';
        ...
        btn.addEventListener('click', () => {
          ...
          // re-render dashboard to reflect new course list
          renderStudentDashboard();
        });
